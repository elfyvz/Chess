...
function getBestMove(fen, callback) {
    let engine = new Worker("../node_modules/stockfish/src/stockfish-nnue-16-single.js");
...
document.querySelectorAll('.piece').forEach(piece => {
    piece.addEventListener('click', (event) => {
        ...
        console.log(`Piece clicked: ${pieceColor} pawn at ${squareId}`);

        if (selectedPiece === pieceElement) {
            console.log('Same piece clicked again, removing highlights');
            removeHighlights();
            selectedPiece = null;
        } else {
            console.log('Different piece clicked, highlighting legal squares');
            removeHighlights();
            selectedPiece = pieceElement;
            highlightLegalSquares(squareId, pieceColor);
        }
    });
});

function highlightLegalSquares(squareId, pieceColor) {
    const legalSquares = getPawnMoves(squareId, pieceColor, boardSquaresArray);
    console.log(`Legal squares for ${pieceColor} pawn at ${squareId}:`, legalSquares);
...
function removeHighlights() {
    console.log('Removing highlights from squares:', highlightedSquares);
